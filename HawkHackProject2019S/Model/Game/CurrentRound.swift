...
class CurrentRound {
    ...
    init(_gameId: String, _roundNumber: Int) {
        self.gameId = _gameId
        self.roundNumber = _roundNumber
    }
...
func saveRoundInFirebase(round: CurrentRound) {
    let ref = firDatabase.child(kGAMESESSIONS).child(round.gameId!).child(kCURRENTGAME)
    ref.setValue(turnDictionaryFrom(round: round))
    print("Finished saving turn \(round.gameId!) in firebase")
}

func saveTurnLocally(round: CurrentRound) {
    UserDefaults.standard.set(turnDictionaryFrom(round: round), forKey: round.gameId!) //save turn with turn's gameId
    UserDefaults.standard.synchronize()
    print("Finished saving turn \(round.gameId!) locally")
}

func turnDictionaryFrom(round: CurrentRound) -> NSDictionary {
    return NSDictionary(
        objects: [round.gameId!, round.roundNumber, round.p1Move, round.p2Move, round.player1HP, round.player2HP],
